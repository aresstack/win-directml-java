...
| Slice | Status |
|-------|--------|
| RELEASE-ARTIFACT-CHECK-1 verify build/distribution artifacts + add release checklist | **done (verification + docs only; no release tag, no publish, no runtime/kernel/format change). Verified tasks: directml-workbench + directml-workbench-launcher both use application+shadow plugins → `run`/`shadowJar`/`distZip`/`distTar`/`installDist`/`build` all exist; README's `:directml-workbench-launcher:run` is valid. Built both shadowJars: directml-workbench-all.jar (~31MB fat jar: app + generation runtime + deps) + directml-workbench-launcher-all.jar (~6.6KB Java8 bootstrapper). **Artifact hygiene verified: the fat jar contains NO model/weight artifacts (no .wdmlpack/.safetensors/.onnx/model dirs; only META-INF/ch/com/org, 15115 files)** — model/ is git-ignored so never committed/bundled; generation weights are user-provided via Download→Convert. Maven boundary confirmed: release.ps1/publish config still publishes only the 4 core library modules; README/release-notes don't sell generation as a Maven stability promise and don't suggest ONNX-Runtime/Python product path. New docs/release-checklist.md (regression → gated smokes → workbench build+manual smoke → fat-jar hygiene grep → Maven boundary → docs → human-go-ahead-only tag/publish + open items). Open items before a real release: no single aggregate Workbench-app distribution task (each module has its own shadowJar/distZip — decide shipping shape). No tag/publish performed. Gemma/Qwen/SmolLM2/T5/Phi unchanged. 4-module regression green.** |
| README-RELEASE-CLOSEOUT-1 refresh README + release notes for model support | **done (docs only; no runtime/kernel/format change). README: softened the "decoder LLMs not part of repo" framing (text generation is an experimental Workbench app feature, not the published embeddings/reranking library); added a "Text generation in the Workbench (experimental)" section (WARP/AUTO/CPU semantics, runnable-model table = Gemma-it/Qwen 0.5B/SmolLM2/T5×4 certified/Phi-3-mini; Phi-3.5 + Qwen 1.5B/3B planned; Download→Convert→Summarize quickstart; known limits; no Python/ONNX Runtime; wdmlpack); fixed the Modules list (added directml-workbench + directml-workbench-launcher; directml-inference = generation runtime not "Phi-3 summarizer"); updated Status-summary rows (Phi-3 row → Workbench generation feature; "Decoder LLMs out of scope" → "ACP/MCP/A2A out of scope"); Maven-Central section no longer calls directml-inference "legacy beta". New docs/release-notes-next.md (draft, no tag/publish): published library unchanged; Workbench generation summary (Gemma/Qwen/SmolLM2/T5-certs/Phi-3-mini heap-light/>2GB reader/WARP-AUTO) + honest known limits. SUPPORTED_MODELS.md + workbench-model-status.md already consistent (closeout slice). Phi-3.5 explicitly NOT pursued. No code change → no test impact; standard run was green at the previous (closeout) commit. Gemma/Qwen/SmolLM2/T5/Phi runtimes unchanged.** |
| PHI3-PRODUCT-CLOSEOUT finalize the Phi-3-mini Workbench product path | **done (docs/lock-in/stale-cleanup only; no runtime/kernel/format change). Verified final Phi state: Phi-3-mini EXPERIMENTAL/runnable from model_phi3.wdmlpack (Phi3PackageLifecycle Convert → READY via light structural open; SummarizerPanel runs it heap-light via Phi3RuntimePackage→Phi3Runtime CPU; no Python/ONNX Runtime, no raw-ONNX in the start path; missing pkg → clear Convert message); Phi-3.5 stays PLANNED. Cleared stale text: GenerationModelRegistry comment ("NOT executable... no wdmlpack compiler" → mini runnable / 3.5 planned-no-weights), SUPPORTED_MODELS.md Phi-3.5 rows ("no wdmlpack compiler" → "same compiler once weights wired"), workbench-model-status.md AUDIT-1 section marked historical/superseded. Added PHI3-PRODUCT-CLOSEOUT status section (workbench-model-status.md). Gating: real compile+smoke stay opt-in (phi3.compile.realModel/phi3.realModel forwarded), std run light, model/phi-3-mini-4k-instruct-onnx git-ignored, runnable-set test includes Phi-3-mini deliberately. Invariants locked by GenerationModelRegistryTest + WorkbenchModelStatusAuditTest + Phi3PackageLifecycleTest + gated Phi3WdmlPackCompilerTest. Gemma/Qwen/SmolLM2/T5 + sidecar unchanged. 4-module regression green. PHI FAMILY = mini runnable/closed; 3.5 PLANNED.** |
| PHI3-RUNTIME-HEAPLIGHT-1 heap-light Phi-3 package load + decode smoke + status flip | **done — DECISION A: Phi-3-mini is Workbench-runnable. Made QuantizedWeight buffer-backed: qweight/zeropoints are ByteBuffers (ONNX/in-memory path wraps byte[] via the kept byte[] constructor; package path passes the mmap'd RuntimeTensor buffers directly = off-heap, ~2GB saved). matvec/matmul bulk-read each INT4 block via absolute ByteBuffer.get(int,byte[],int,int) (thread-safe, keeps SIMD dequant fast). Phi3RuntimePackage.quantized now references the mmap buffers (no byte[] copy); embed/scales stay float[] (~450MB). weights() heap dropped ~3GB→~783MB. Re-added the gated decode smoke (Phi3WdmlPackCompilerTest, -Dphi3.realModel=true) — GREEN at the STANDARD 2g heap (heapUsed≈783MB, prompt "Name one primary color" → "Red"). SummarizerPanel.runPhi3Summarizer rewired: runs Phi-3 from model_phi3.wdmlpack via Phi3RuntimePackage→Phi3Runtime (CPU, native Java/DirectML, no Python/ONNX Runtime) instead of raw ONNX (removed validatePhi3ModelFiles + the ONNX Phi3Summarizer path); prompt rendered via PromptStrategies (PHI3 chat template). **Status flip: Phi-3-mini PLANNED→EXPERIMENTAL/runnable; Phi-3.5 stays PLANNED.** Updated GenerationModelRegistry (note: runs from model_phi3.wdmlpack, no Python/ORT) + GenerationModelRegistryTest (registryContainsPhi3MiniExperimentalRunnable) + WorkbenchModelStatusAuditTest (Phi-3-mini in executable list + runnable set; phiStatusIsHonest; removed from PLANNED list) + docs (plan banner, workbench-model-status row, SUPPORTED_MODELS.md, this row). wdmlpack format UNCHANGED; ONNX Phi3Weights.load + Phi3GpuKernels (materialize byte[] via kept qWeight() accessor) still work; small Qwen/T5/SmolLM2 packages + GPU path unaffected. Gemma/Qwen/SmolLM2/T5 unchanged. 4-module regression green; gated smoke green at 2g.** |
| PHI3-WORKBENCH-RUNNABLE-1 wire Phi-3 lifecycle/gate (decision C: runtime smoke deferred) | **done (lifecycle/gate/convert wiring + light lifecycle test; NO status flip, NO runtime decode smoke committed — Phi-3 stays PLANNED). New `Phi3PackageLifecycle` (hasCompiler=true; convert via Phi3WdmlPackCompiler; READY detection via LIGHT structural Phi3RuntimePackage.open, NOT the ~3GB weights()) replaces CompilerMissingLifecycle(PHI3) in WorkbenchArtifactGate + DefaultModelArtifactService.createDefault + DownloadPanel addPhi3Row → Phi-3 is now downloadable AND convertible to model_phi3.wdmlpack, compiled package detected READY. Tests: Phi3PackageLifecycleTest (missing→not-ready, synthetic compiled→PACKAGE_VALID/ready) + updated WorkbenchArtifactGateTest (Phi-3 no-package now → "Use Download → Convert", not "compiler not implemented"; still fails fast, never runs raw ONNX). **DECISION C: the runtime decode smoke (package→weights()→Phi3Runtime→decode) + the PLANNED→EXPERIMENTAL status flip are DEFERRED — the full ~3GB weights() reconstruction + 2.39GB package mmap over-commits this host's ~6GB free RAM in the forked test JVM (OS-killed, not a clean failure), so the smoke couldn't be validated and a JVM-crashing gated test is unsafe to commit. Needs a heap-light Phi runtime-package loader (Phi3Weights/QuantizedWeight reference mmap'd buffers instead of copying ~3GB to heap) → PHI3-RUNTIME-HEAPLIGHT-1, then the flip.** Phi-3-mini + Phi-3.5 stay PLANNED. No status/registry change; no Python/ONNX-Runtime. Reverted the COMPILER-2 conditional heap (back to plain 2g) + removed phi3.realModel forwarding. Gemma/Qwen/SmolLM2/T5 unchanged. 4-module regression green. Docs updated.** |
| WDMLPACK-LARGE-READER-1 read >2GB wdmlpack packages | **done (shared reader fix only; no format change, no Workbench wiring, no status flip — Phi-3 stays PLANNED). Root cause: WdmlPackWriter.readManifest mmapped the WHOLE file with int offsets + rejected fileSize>Integer.MAX_VALUE; WdmlPackReader.mapPayloadTensors did the same (whole-file map + Math.toIntExact slicing). readHeader was already positional/long-safe; per-tensor payloads are <2GB. Fix: (1) readManifest now reads only the manifest region positionally via channel.read(buffer, manifestOffset) — no whole-file map, no size cap (manifest is small); removed the dead private mmap overload + MappedByteBuffer import. (2) mapPayloadTensors now maps EACH tensor's region individually with a long file offset `channel.map(READ_ONLY, payloadOffset_abs, payloadLength)` (per-tensor length<2GB, mappings survive channel close as before); removed the fileSize>MAX guard. Format UNCHANGED (binary layout v1 identical); the COMPILER-2 2.39GB package stays valid. **Result: real Phi-3-mini ~2.39GB package now RELOADS — Phi3RuntimePackage.open + runtimeTensorCatalog() validate config + 711 role tensors (gated test updated from assertThrows to a real structural reload).** Small Qwen/T5/SmolLM2/Gemma packages unchanged (4-module regression green). SafeTensorsReader left as-is (different format, out of scope). **Decision A: real >2GB package read + Phi3RuntimePackage loads it (structural).** Remaining before runnable = full weights() in-memory reconstruction footprint (~3GB) + Workbench lifecycle → PHI3-WORKBENCH-RUNNABLE-1. Gated test: `:directml-inference:test --tests '*Phi3WdmlPackCompilerTest' -Dphi3.compile.realModel=true [-Dphi3.testModelDir=...]`. Docs updated. Workbench stays PLANNED.** |
| PHI3-WDMLPACK-COMPILER-2 make the real Phi-3-mini compile heap-safe | **done (compiler memory fix only; no Workbench wiring, no status flip — Phi-3 stays PLANNED; no format change). Diagnosis: the COMPILER-1 crash was the ~3GB load peak (Phi3Weights.load materializes ~2.1GB INT4 byte[] + ~394MB fp32 embed); a standalone diag confirmed the compile works at 6g (load 4.3s/~3GB, writes 2.39GB pkg). Fix: `Phi3WdmlPackCompiler.compile` now STREAMS each tensor straight from the mmap'd model.onnx.data via new `Phi3Weights.planLayout`→`Phi3WeightLayout` (role→ExtRef/inline refs, no payload bytes), copying raw INT4 / converting fp16→fp32 in bounded 64KB buffers — never building a full Phi3Weights. Shared serialization core writeContainer() backs both compile (streaming) + writePackage (in-memory synthetic). **Result: gated real Phi-3-mini compile now runs within the STANDARD 2g test heap and produces a real model_phi3.wdmlpack — 711 tensors, 32 layers, ~2.39GB.** INT4 byte-exact preserved; format unchanged (synthetic round-trip still green). **Known remaining blocker (reload, not compile): the ~2.39GB package exceeds the shared WdmlPackReader Integer.MAX_VALUE whole-file mmap limit ("too large for the current lightweight mmap reader") → reloading >2GB packages needs a separate shared-reader slice (positional manifest read + windowed payload mapping); touches Qwen/T5/SmolLM2 so out of scope here.** Gated test asserts the heap-safe compile + documents the >2GB reader limit (assertThrows). Removed temp diag + task; test heap stays 2g. **Decision A (compile heap-safe + produces wdmlpack); reload >2GB = documented follow-up.** Gemma/Qwen/SmolLM2/T5 unchanged. 4-module regression green. Next: shared >2GB wdmlpack-reader slice, then PHI3-WORKBENCH-RUNNABLE-1.** |
| PHI3-WDMLPACK-COMPILER-1 build the minimal Phi-3 ONNX→wdmlpack compiler foundation | **done (compiler + package foundation only; NO Workbench wiring, NO status flip — Phi-3 stays PLANNED/gate-blocked; no runtime/kernel change). New: `Phi3OnnxModelSource` (ONNX import boundary, reuses Phi3Weights extraction, no Python/ORT), `Phi3WdmlPackRoles` (role catalog: INT4 projections as qweight[N,K,blockSize]/scales/zeropoints triplets, fp32 vectors), `Phi3CompileOptions`, `Phi3WdmlPackCompiler` (extracts Phi3Weights → serializes role-named RuntimeTensors into model_phi3.wdmlpack via WdmlPackWriter), `Phi3RuntimePackage` (opens via shared WdmlPackReader, reconstructs Phi3Config + Phi3Weights). Runtime seam = `Phi3Weights.ofRecords` (package-backed factory, no file handles; null-safe close) so Phi3Runtime is unchanged. Tensor roles written: embed_tokens, cos_cache, sin_cache, final_norm, lm_head(+triplet), per-layer input_norm/post_norm/attn_out_scale/mlp_out_scale + q/k/v/o/gate_up/down (triplets). INT4 MatMulNBits preserved byte-exact (proven). config in manifest; tokenizer referenced (tokenizer.json sibling), not embedded. Test: Phi3WdmlPackCompilerTest synthetic round-trip (non-gated, byte-exact, passes at 2g) + gated real compile (-Dphi3.compile.realModel=true). **Real Phi-3-mini compile NOT yet verified end-to-end on this host** (gated run crashes forked test JVM ~15s in even at 5–7g; needs streaming compile / mmap investigation → PHI3-WDMLPACK-COMPILER-2). Forwarded phi3.compile.realModel/phi3.testModelDir in build.gradle; added model/phi-3-mini-4k-instruct-onnx to .gitignore. **Decision A (foundation steht); next slice can wire the Workbench lifecycle, after the real compile is confirmed.** Gemma/Qwen/SmolLM2/T5 unchanged. 4-module regression green.** |
| PHI-WDMLPACK-COMPILER-AUDIT-1 spec the Phi-3 wdmlpack compiler | **done (spec/docs only; no compiler, no runtime/kernel change, no status flip, no format change). Audited the Phi code (Phi3InferenceEngine/Phi3Weights/Phi3Runtime/GpuKernels/Tokenizer/Config — library/sidecar, native Java/DirectML over ONNX-format weights, no Python/ORT) vs the wdmlpack pipeline. Phi-3-mini source = config.json+tokenizer.json+model.onnx+model.onnx.data (INT4 MatMulNBits Q4G128 + fp16); Phi3Weights extracts via ONNX graph-topology tracing + custom external-data offset parser. Gap: Workbench gate = CompilerMissingLifecycle(PHI3) (no compiler) so it always blocks. **Decision B: a real new compiler is needed but the runtime is largely reusable.** Reusable: whole Phi runtime (Phi3Runtime/GpuKernels/Pipeline/Tokenizer/Config/shaders), shared wdmlpack/RuntimeTensor infra, and — crucially — Qwen ALREADY does ONNX(INT4)→wdmlpack (QwenOnnxModelSource/QwenWdmlPackCompiler/QwenPackageLifecycle), so the INT4 wdmlpack format is defined/shipping and is the template. New (mirror Qwen/SmolLM2): Phi ONNX ModelSource + role mapping + Phi3WdmlPackCompiler + Phi3RuntimePackage + package-backed Phi3Weights loader (only real runtime seam) + Phi3PackageLifecycle + gate/download wiring + status flip + gated cert. Phi-3.5 = same path once weights published (no new mapping expected). Doc: new docs/phi3-wdmlpack-compiler-plan.md + workbench-model-status.md pointer. Suggested follow-ups: PHI3-WDMLPACK-COMPILER-1 then PHI3-WORKBENCH-RUNNABLE-1. Workbench status stays PLANNED. Gemma/Qwen/SmolLM2/T5 unchanged. 4-module regression green.** |
| WORKBENCH-MODEL-STATUS-CLOSEOUT-1 finalize the model-status audit | **done (verification + docs + lock-in only; no runtime/kernel/format/downloader/UI change). Verified the full GenerationModelRegistry matrix is honest & consistent: runnable (EXPERIMENTAL) = gemma-3-270m-it, Qwen 0.5B, SmolLM2 135M/360M, all 4 T5 (t5-small/flan/codet5-small/codet5-base-multi-sum) — each with a verified executable Workbench path; PLANNED/blocked = Qwen 1.5B/3B, gemma-3-270m base, Phi-3-mini, Phi-3.5. No EXPERIMENTAL model is gate-blocked (gate blocks only PHI3, now PLANNED). Stale-wording sweep (Runtime integration is planned / not executable yet / not implemented yet / probe / download only / uses ONNX Runtime / false Python/native-WARP): remaining hits all legitimate (honest PLANNED guard message, historical chronicle, opt-in SmolLM2 probe diagnostic's own message, "no Python/ONNX Runtime" negation). New lock-in WorkbenchModelStatusAuditTest.runnableSetIsExactlyTheKnownProductModels pins the exact runnable set. Docs: workbench-model-status.md closeout section. Real-model certs stay opt-in; model/... git-ignored. Gemma/Qwen/SmolLM2/T5/Phi runtimes all unchanged. 4-module regression green. Model-status strand CLOSED.** |
| PHI3-PRODUCT-AUDIT-1 audit + honest Phi-3 workbench status | **done (audit/labels/docs only; no runtime change). Key finding: Phi-3-mini was registry-EXPERIMENTAL (runnable by status) but the Workbench artifact gate hard-blocks it — `SummarizerPanel.runPhi3Summarizer` → `WorkbenchArtifactGate.requireExecutablePackage(PHI3)` → `CompilerMissingLifecycle` (no wdmlpack compiler) → `inspect()` always PACKAGE_COMPILER_MISSING/executable=false → `ready()`=false → always throws. So Phi-3 is NOT executable in the Workbench (false promise). Downgraded Phi-3-mini EXPERIMENTAL→PLANNED (Phi-3.5 already PLANNED) so the panel guard blocks both upfront with the honest "selectable but not executable yet". Corrected misleading notes: registry claimed "Uses ONNX Runtime GenAI" — wrong, there is no ORT/GenAI/Python; the real `Phi3InferenceEngine` is a native Java/DirectML decoder over ONNX-format weights (model.onnx+model.onnx.data), available via the sidecar `summarize` path / library/tests only (sidecar uses EmbeddingModelRegistry, stays EXPERIMENTAL there — genuinely runs, no gate). Reworded GenerationModelRegistry (both Phi) + EmbeddingModelRegistry (both Phi, ORT→native) notes. No Python on any Phi path. Tests: GenerationModelRegistryTest (Phi-3 PLANNED/not-runnable), WorkbenchModelStatusAuditTest.phiModelsAreNotRunnableAndCarryHonestNotes + moved Phi-3-mini to the PLANNED list. Docs: workbench-model-status.md (Phi rows + audit section), SUPPORTED_MODELS.md (Phi Workbench-not-executable + §3.2 smoke-test banner). Making Phi executable = future wdmlpack-compiler slice (out of scope). Gemma/Qwen/SmolLM2/T5 unchanged. 4-module regression green.** |
| T5-PRODUCT-CLOSEOUT finalize curated T5 product certification | **done (docs/lock-in/labels only; no runtime/kernel/format change). Final product status: all four curated T5 models (t5-small, flan-t5-small, codet5-small, codet5-base-multi-sum) are real-certified (CPU==WARP, greedy; CERT-1..4); CPU=always-validated reference, WARP/AUTO=mixed DirectML path certified against CPU for the four; no Python; .wdmlpack mandatory + Download→Convert prerequisite. Certification scope/product boundary documented: covers exactly these four — any other/new T5 model needs its own gated cert (no blanket guarantee). Cleared the last stale standalone "not yet correctness-certified" wording: workbench-model-status.md AUDIT-1 line now points forward to the certification; T5Runtime class javadoc + UNSUPPORTED_MESSAGE javadoc updated to say the curated four are certified (boundary uncertified only for other T5 models). T5 not in SUPPORTED_MODELS.md (nothing to change). Real-model + synthetic certs stay opt-in (cert asserts token+text parity when run); model/... artifacts git-ignored. Added T5-PRODUCT-CLOSEOUT subsection to workbench-model-status.md + closeout banner to t5-realmodel-cert.md. Gemma/Qwen/SmolLM2/Phi unchanged. 4-module regression green. ENTIRE curated T5 family product path = DONE.** |
