 ...
 *
 * <p>This is a building-block composition (CPU readback between kernels), the parity step before the
 * fused single-submission product pipeline. Requires {@link WindowsBindings#isSupported()}.</p>
 */
public final class Gemma3WarpLayer implements AutoCloseable {
    ...
    private final Gemma3WarpAttentionScoresKernel scoresKernel;
    private final Gemma3WarpSoftmaxKernel softmaxKernel;
    private final Gemma3WarpAttentionValueKernel valueKernel;
    ...
    public Gemma3WarpLayer(WindowsBindings wb, Gemma3Config config, int layerIndex,
        ...
        this.scoresKernel = new Gemma3WarpAttentionScoresKernel(wb);
        this.softmaxKernel = new Gemma3WarpSoftmaxKernel(wb);
        this.valueKernel = new Gemma3WarpAttentionValueKernel(wb);
        this.mlp = new Gemma3WarpMlp(wb, hidden, config.intermediateSize(),
                w.gateProj(), w.upProj(), w.downProj());
    }
    ...
    public float[][] forward(float[][] state) throws WindowsNativeException {
        ...
        for (int t = 0; t < s; t++) {
            float[] normed = rmsNorm.normalize(state[t], inputLayerNorm, eps);
            float[] qt = qProj.project(normed);
            float[] kt = kProj.project(normed);
            float[] vt = vProj.project(normed);
            qt = qkNorm.normalizeHeads(qt, numHeads, headDim, qNorm, eps);
            kt = qkNorm.normalizeHeads(kt, numKvHeads, headDim, kNorm, eps);
            qt = rope.applyToHeads(qt, numHeads, headDim, t, theta);
            kt = rope.applyToHeads(kt, numKvHeads, headDim, t, theta);
            q[t] = qt;
        ...
        for (int t = 0; t < s; t++) {
            ...
            float[] scores = scoresKernel.scores(q[t], kFlat, numHeads, numKvHeads, headDim, s, t, firstValid, scale);
            float[] prob = softmaxKernel.softmaxRows(scores, numHeads, s);
            float[] context = valueKernel.aggregate(prob, vFlat, numHeads, numKvHeads, headDim, s);
            float[] attnProj = oProj.project(context);
            attnProj = rmsNorm.normalize(attnProj, postAttentionLayerNorm, eps);
        ...
        for (int t = 0; t < s; t++) {
            float[] ff = rmsNorm.normalize(state[t], preFeedforwardLayerNorm, eps);
            float[] down = mlp.mlp(ff);
            down = rmsNorm.normalize(down, postFeedforwardLayerNorm, eps);
            for (int i = 0; i < hidden; i++) {
                state[t][i] += down[i];
            }
        }
